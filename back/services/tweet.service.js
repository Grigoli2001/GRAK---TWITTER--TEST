...
const getTweetsByCategory = async (req, res) => {
  ...
  if (!categoryConditions[category])
    return res.status(statusCode.badRequest).json({
      status: "fail",
...
const createTweet = async (req, res) => {
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (tweetPoll) {
        ...
        if (poll_options?.length < 2 || poll_options?.length > 4) {
          ...
          poll_options.some((option) => !option.text || option.text.length > 25)
        ) {
          throw Error("Invalid poll data");
        } else if (
          !tweetPoll.duration ||
          tweetPoll.duration.days < 1 ||
          tweetPoll.duration.hours < 1 ||
          tweetPoll.duration.minutes < 1 ||
          tweetPoll.duration.days > 7 ||
          tweetPoll.duration.hours > 24 ||
          tweetPoll.duration.minutes > 60
        ) {
          throw Error("Invalid poll duration");
        }
        const pollExpiration = new Date(
          Date.now() +
            tweetPoll.duration.days * 24 * 60 * 60 * 1000 +
            tweetPoll.duration.hours * 60 * 60 * 1000 +
            tweetPoll.duration.minutes * 60 * 1000
        );

        const poll = new pollModel({
          options: poll_options,
          poll_end: pollExpiration,
        });
    ...
    const user = await pool.query(
      `SELECT id, username, name, profile_pic FROM users WHERE id = $1`,
      [req.user.id]
    );
...
const updateTweet = async (req, res) => {
  try {
    const tweets = await tweetModel.findOneAndUpdate(
...
const deleteTweet = async (req, res) => {
  try {
    const tweets = await tweetModel.findOneAndDelete({
      tweet_id: req.params.id,
    });
...
const getReplies = async (req, res) => {
  try {
    const replies = await tweetQuery(
      { reference_id: new ObjectId(req.params.id), tweetType: "reply" },
      req.user.id
    );
    if (!replies) {
    ...
    res.status(404).json({
      status: "fail",
      message: "error finding the tweet",
      err,
    });
  }
};
...
const likeTweet = async (req, res) => {
  try {
    ...
    const currentLikeInteraction = await Interaction.findOne({
      tweet_id: tweetId,
      userId: req.user.id,
    ...
    if (currentLikeInteraction) {
      const outOfSync =
        (isLiked && currentLikeInteraction.is_deleted) ||
        (!isLiked && !currentLikeInteraction.is_deleted);
      ...
      if (outOfSync) {
        ...
        likeInteraction = await Interaction.findOneAndUpdate(
          {
            tweet_id: req.body.tweetId,
            userId: req.user.id,
            interactionType: "like",
          },
          { is_deleted: !currentLikeInteraction.is_deleted },
          { new: true }
        );
      }
    } else {
...
const bookmarkTweet = async (req, res) => {
  try {
    ...
    const currentBoomarkInteraction = await Interaction.findOne({
      tweet_id: tweetId,
      userId: req.user.id,
    ...
    if (currentBoomarkInteraction) {
      const outOfSync =
        (isBookmarked && currentBoomarkInteraction.is_deleted) ||
        (!isBookmarked && !currentBoomarkInteraction.is_deleted);
      ...
      if (outOfSync) {
        ...
        bookmarkedInteraction = await Interaction.findOneAndUpdate(
          {
            tweet_id: tweetId,
            userId: req.user.id,
            interactionType: "bookmark",
          },
          { is_deleted: !currentBoomarkInteraction.is_deleted },
          { new: true }
        );
      }
    } else {
      bookmarkedInteraction = new Interaction({
        tweet_id: tweetId,
        userId: req.user.id,
        interactionType: "bookmark",
      });
