    ...
    if (!email || !name || !password) {
        ...
        try {
            ...
            const addUser = await client.query(
                `INSERT INTO users(email,name, username, password,dob)
           VALUES ($1,$2,$3, crypt($4, gen_salt('bf')), $5);`,
                [email, name ,username, password, new Date().toJSON().slice(0, 10)]
            );
...
const login = async (req, res) => {
    ...
    if (!userInfo || !password) {
        ...
        try {
            ...
            return res.status(statusCodes.success).json({token, username: user.rows[0].username });
