...
const changeFollowStatus = async (req, res) => {
    ...
    if (!username) {
        return res.status(statusCodes.badRequest).json({ message: "Missing user to follow" });
    }
    ...
    if (!following.rowCount) {
        return res.status(statusCodes.badRequest).json({ message: "Invalid user to follow" });
    }

    if (following.rows[0].id === user_id) {
        return res.status(statusCodes.badRequest).json({ message: "You cannot follow yourself" });
    }
...
const changeUsername = async (req, res) => {
    ...
    if (usernameExists.rowCount) {
        return res.status(statusCodes.badRequest).json({ message: "Username already exists" });
    }
