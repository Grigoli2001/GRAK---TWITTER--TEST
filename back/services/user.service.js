...
const getUserByUsername = async (req, res) => {
  ...
  try {
    ...
    let postcount = await tweetModel.countDocuments({
      userId: user.id,
...
const unblockUser = async (req, res) => {
  ...
  try {
    ...
    res
      .status(statusCodes.serverError)
      .json({ message: "Error unblocking user" });
  }
};
...
const checkBlocked = async (req, res, next) => {
  try {
    ...
    if (!otherUserId) {
      return res
        .status(statusCodes.badRequest)
        .json({ message: "Missing fields (otherUser)" });
    }

    if (otherUserId === req.user.id) {
      return res
        .status(statusCodes.badRequest)
        .json({ message: "Cannot perform action on self" });
    }
    ...
    res
      .status(statusCodes.serverError)
      .json({ message: "Error checking blocked" });
  }
};
