    ...
    return (
        ...
        <NavModal backTo={backTo}>
            ...
            <div className={ cn('flex items-center justify-center', 
                ...
                {type === 'cover' ?
                <img src={user.cover} alt="cover" className='w-full max-h-full object-cover' />
                :
...
const EditProfileModal = ({ backTo }) => {
    ...
    const [ month, setMonth] = useState(userDob.toLocaleDateString('en-US', {month: 'long'} ) )
    const [ year, setYear] = useState(isNaN(userDob.getFullYear() ))

    console.log(day, month, year)

    // todo memoize
    const [dayOptions, setDayOptions] = useState(getDaysInMonth( year, months.indexOf(month)))
        ...
        <NavModal backTo={backTo}>
            ...
            <div className='block w-fit h-[80%] min-w-[40%] overflow-y-auto bg-white rounded-2xl'>
                ...
                <div className='flex items-center p-4'> 
                    ...
                    <Button variant="dark" size="sm" className="ml-auto" onClick={updateProfile}>
                        Save
                    </Button>
                </div>
                ...
                <div className="h-48 w-full relative mb-6 bg-slate-300 col-span-full flex items-center justify-center gap-x-4 bm-8">
                    ...
                    <div className="z-20 flex items-center justify-center gap-x-4">
                        <Button variant={'icon'} size={'icon-sm'} tooltip={'Change cover photo'} className="bg-black/50 hover:bg-black/30">
                            <label htmlFor="profile-cover">
                                <CICameraPlus />
                                <input id="profile-cover" name="profile-cover" className="hidden" type="file" accept="image/*" />
                            </label>
                        </Button>

                        {
                            user.cover && 
                            <Button variant="icon" size='icon-sm' tooltip='Remove cover photo' className="bg-black/50 hover:bg-black/30 text-white">
                                <FaXmark />
                            </Button>
                        }
                    </div>
                    ...
                    <div className="absolute left-5 z-20 bottom-[-25%]">
                        ...
                        <div className="top-0 z-[1000] rounded-full absolute p-1 flex items-center justify-center bg-slate-700 bg-opacity-25 h-28 w-28">
                            <Button variant={'icon'} size={'icon-sm'} tooltip={'Change profile photo'} className="bg-black/50 hover:bg-black/30">
                                <label htmlFor="profile-photo">
                                    <CICameraPlus />
                                    <input id="profile-photo" name="profile-photo" className="hidden" type="file" accept="image/*" />
                                </label>
                            </Button>
                        </div>
                    <ExtAvatar user={user.avatar} size="lg" className={"rounded-[50%] bg-white p-1 h-28 w-28 object-cover shadow-inner"} />
                ...
                <div className='p-4 mt-10 grid gap-y-3'>
                    ...
                    <div>
                        ...
                        <div className='flex gap-x-2'>
                            ...
                            <OptionSelector title="Day" options={dayOptions} name={'day'} defaultValue={day} 
                            onChange={(e, newVal) => setDay(newVal)}/>

                            <OptionSelector title="Month" options={months} defaultValue={month} 
                            onChange={(e, newVal) => {setMonth(newVal); setDayOptions(
                                getDaysInMonth(year, months.indexOf(newVal))
                                )}} />

                            <OptionSelector title="Year" options={years} defaultValue={year} 
                            onChange={(e, newVal) => {setYear(newVal);
                                setDayOptions(
                                    getDaysInMonth(newVal, months.indexOf(month))
                                ) 
